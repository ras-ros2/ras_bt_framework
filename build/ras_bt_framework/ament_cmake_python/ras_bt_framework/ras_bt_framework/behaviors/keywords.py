...
from ras_bt_framework.behaviors.gen_primitives import move2pose as move2poseClass
from ras_bt_framework.behaviors.gen_primitives import Move as MoveClass
from ras_bt_framework.behaviors.gen_primitives import Pick as PickClass
from ras_bt_framework.behaviors.gen_primitives import Place as PlaceClass
...
def Move(pose=None, from_pose=None, to_pose=None, **kwargs):
    ...
    if from_val is not None and to_val is not None:
        ...
        if from_val not in target_pose_map.pose_map:
            print(f"ERROR: Invalid 'from' pose: '{from_val}'. Available poses: {list(target_pose_map.pose_map.keys())}")
            raise ValueError(f"Invalid 'from' pose: '{from_val}'. Pose not found in target_pose_map.")
        if to_val not in target_pose_map.pose_map:
            print(f"ERROR: Invalid 'to' pose: '{to_val}'. Available poses: {list(target_pose_map.pose_map.keys())}")
            raise ValueError(f"Invalid 'to' pose: '{to_val}'. Pose not found in target_pose_map.")
        
        # Create a sequence using move2pose to ensure identical behavior
    ...
    # Legacy support for just 'pose'
        ...
        if pose not in target_pose_map.pose_map:
            print(f"ERROR: Invalid 'pose': '{pose}'. Available poses: {list(target_pose_map.pose_map.keys())}")
...
def Pick(above=None, at=None, **kwargs):
    ...
    if above_val not in target_pose_map.pose_map:
        print(f"ERROR: Invalid 'above' pose: '{above_val}'. Available poses: {list(target_pose_map.pose_map.keys())}")
    ...
    if at_val not in target_pose_map.pose_map:
        print(f"ERROR: Invalid 'at' pose: '{at_val}'. Available poses: {list(target_pose_map.pose_map.keys())}")
