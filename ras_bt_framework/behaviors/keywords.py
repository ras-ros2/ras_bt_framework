...
def joint_state(joints:list):
    """
    Create a MoveToJointState behavior with the given joint values.
    Args:
        joints (list): List of joint values in radians. Must match the number of joints for the active robot.
    Returns:
        MoveToJointState: A behavior node for moving to the specified joint state.
    Raises:
        ValueError: If the number of joints doesn't match the active robot's configuration.
    """
    from ras_common.config.loaders.lab_setup import LabSetup
    LabSetup.init()
    # Get joint names from the active robot's configuration
    joint_names = list(LabSetup.robot_config.home_joint_state.keys())
    if len(joints) != len(joint_names):
        raise ValueError(f"Invalid number of joints {len(joints)} for robot {LabSetup.robot_name}. Expected {len(joint_names)} joints.")
    joint_state = ",".join([f"{joint_names[i]}:{joints[i]}" for i in range(len(joints))])
    return MoveToJointState(i_joint_state=joint_state)
keyword_mapping = {
    "rotate": rotate,
