...
{
    BTExecutor::BTExecutor(std::shared_ptr<BT::BehaviorTreeFactory> bt_factory, const rclcpp::NodeOptions &options) : rclcpp::Node("bt_executor", options), bt_factory_(bt_factory), bt_session_(nullptr)
    {
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Received goal request with id %s", goal->bt_path.c_str());
        (void)uuid;
        (void)goal;
    ...
    BTExecutor::BTSession::BTSession(const std::string &_bt_path, BTIGoalHandlePtr _goal_handle) : bt_path(_bt_path), goal_handle(_goal_handle), first(true) {};
    ...
    void BTExecutor::bt_tick_handler(const TickSrv_t::Request::SharedPtr request, TickSrv_t::Response::SharedPtr response)
    {
        ...
        {
        }; // TODO: Implement this for dynamic task execution
