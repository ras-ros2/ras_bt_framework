...
namespace ras_bt_framework
{
   ...
   virtual BT::NodeStatus tick() override {
        ...
        if (!getInput("above", above_pose_id)) {
            std::cerr << "Pick: 'above' parameter missing" << std::endl;
            return BT::NodeStatus::FAILURE;
        }
        
        if (!getInput("at", at_pose_id)) {
            std::cerr << "Pick: 'at' parameter missing" << std::endl;
            return BT::NodeStatus::FAILURE;
        }
        
        std::cout << "Picking: moving from above " << above_pose_id << " to " << at_pose_id << " and back" << std::endl;
        ...
        // 2. Open gripper
        // 3. Use move2pose's logic to move to at_pose_id
        // 4. Close gripper
        // 5. Use move2pose's logic to move back to above_pose_id
